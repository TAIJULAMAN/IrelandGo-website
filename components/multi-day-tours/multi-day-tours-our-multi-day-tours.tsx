...
import { ChevronLeft, ChevronRight, Star, Clock, ArrowRight, ArrowDown } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-0 lg:px-0 xl:px-0 relative z-10">
        ...
        {!isLoading && pagedTours.length > 0 && (
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            {pagedTours.map((t: any) => renderTourCard(t))}
          </div>
        )}
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="grid md:hidden grid-cols-2 gap-3 sm:gap-6">
            {filtered.slice(0, mobileVisibleCount).map((t: any) => renderTourCard(t))}
          </div>
        )}

        {/* Mobile See More Button */}
        {!isLoading && mobileVisibleCount < filtered.length && (
